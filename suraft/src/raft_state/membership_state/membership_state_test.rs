...
fn test_membership_state_update_committed() -> anyhow::Result<()> {
    let new = || {
        MembershipState::<UTConfig>::new(
            Arc::new(EffectiveMembership::new(Some(log_id(2, s(1), 2)), m1())),
            Arc::new(EffectiveMembership::new(Some(log_id(3, s(1), 4)), m123_345())),
        )
    };
    ...
    {
        let mut x = new();
        let res = x.update_committed(Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m12())));
        assert!(res.is_none());
        assert_eq!(&Some(log_id(2, s(1), 2)), x.committed().log_id());
        assert_eq!(&Some(log_id(3, s(1), 4)), x.effective().log_id());
    }

    // Update committed, not effective.
    {
        let mut x = new();
        let res = x.update_committed(Arc::new(EffectiveMembership::new(Some(log_id(2, s(1), 3)), m12())));
        assert!(res.is_none());
        assert_eq!(&Some(log_id(2, s(1), 3)), x.committed().log_id());
        assert_eq!(&Some(log_id(3, s(1), 4)), x.effective().log_id());
    }
    ...
    {
        let mut x = new();
        let res = x.update_committed(Arc::new(EffectiveMembership::new(Some(log_id(3, s(1), 4)), m12())));
        assert_eq!(Some(x.effective().clone()), res);
        assert_eq!(&Some(log_id(3, s(1), 4)), x.committed().log_id());
        assert_eq!(&Some(log_id(3, s(1), 4)), x.effective().log_id());
    ...
    {
        let mut x = new();
        let res = x.update_committed(Arc::new(EffectiveMembership::new(Some(log_id(2, s(1), 5)), m12())));
        assert_eq!(Some(x.effective().clone()), res);
        assert_eq!(&Some(log_id(2, s(1), 5)), x.committed().log_id());
        assert_eq!(&Some(log_id(2, s(1), 5)), x.effective().log_id());
        assert_eq!(&m12(), x.effective().membership());
    }
...
fn test_membership_state_append() -> anyhow::Result<()> {
    ...
    assert_eq!(&Some(log_id(3, s(1), 4)), ms.committed().log_id());
    assert_eq!(&Some(log_id(4, s(1), 5)), ms.effective().log_id());
...
fn test_membership_state_commit() -> anyhow::Result<()> {
    ...
    {
        let mut ms = new();
        ms.commit(&Some(log_id(1, s(1), 1)));
        assert_eq!(&Some(log_id(2, s(1), 2)), ms.committed().log_id());
        assert_eq!(&Some(log_id(3, s(1), 4)), ms.effective().log_id());
    }

    // Equal committed
    {
        let mut ms = new();
        ms.commit(&Some(log_id(2, s(1), 2)));
        assert_eq!(&Some(log_id(2, s(1), 2)), ms.committed().log_id());
        assert_eq!(&Some(log_id(3, s(1), 4)), ms.effective().log_id());
    }
...
fn test_membership_state_truncate() -> anyhow::Result<()> {
...

    {
        ...
        assert_eq!(&Some(log_id(2, s(1), 2)), res.unwrap().log_id());
        assert_eq!(&Some(log_id(2, s(1), 2)), ms.committed().log_id());
        assert_eq!(&Some(log_id(2, s(1), 2)), ms.effective().log_id());
    }
