...
///
/// This trait is used to define types that can be directly upgraded from older versions of suraft
/// to newer versions. For example, `LogId` can be upgrade: in suraft 0.7, `LogId` is `(term,
/// index)`, which is upgraded to `(CommittedLeaderId, index)` in suraft 0.8.
