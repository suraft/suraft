...
where C: RaftTypeConfig
{
    ...
    /// policy.
    ///
    /// This method is called after building a snapshot, because suraft only purge logs that are
    ...
    #[tracing::instrument(level = "debug", skip_all)]
    pub(crate) fn update_purge_upto(&mut self, purge_upto: LogId) {
    ...
    #[tracing::instrument(level = "debug", skip_all)]
    pub(crate) fn calc_purge_upto(&self) -> Option<LogId> {
        let st = &self.state;
