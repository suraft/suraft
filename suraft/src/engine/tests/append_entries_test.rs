    ...
    assert_eq!(
        vec![
            ...
            Command::UpdateIOProgress {
                ...
                io_id: IOId::new_log_io(Vote::new(2, s(1)).into_committed(), Some(log_id(0, 0)))
            }
        ],
        eng.output.take_commands()
    );
...
fn test_append_entries_prev_log_id_conflict() -> anyhow::Result<()> {
    ...
    assert_eq!(
        vec![
            ...
            Command::TruncateLog { since: log_id(1, 2) },
        ],
        eng.output.take_commands()
    );
...
fn test_append_entries_prev_log_id_is_committed() -> anyhow::Result<()> {
    ...
    let res = eng.append_entries(&Vote::new_committed(2, s(1)), Some(log_id(0, 0)), vec![
        blank_ent(1, 1),
        blank_ent(2, 2),
    ]);
...
fn test_append_entries_conflict() -> anyhow::Result<()> {
    ...
    let resp = eng.append_entries(&Vote::new_committed(2, s(1)), Some(log_id(1, 1)), vec![
        blank_ent(1, 2),
    ...
    assert_eq!(
        vec![
            ...
            Command::TruncateLog { since: log_id(2, 3) },
            Command::AppendInputEntries {
                committed_vote: Vote::new(2, s(1)).into_committed(),
