...
fn test_append_entries_vote_is_rejected() -> anyhow::Result<()> {
    ...
    let res = eng.append_entries(&Vote::new(1, s(1)), None, Vec::<Entry<UTConfig>>::new());

    assert_eq!(Err(RejectAppendEntries::ByVote(Vote::new(2, s(1)))), res);
    assert_eq!(
        &[
            log_id(1, s(1), 1), //
            log_id(2, s(1), 3),
    ...
    assert_eq!(Some(&log_id(2, s(1), 3)), eng.state.last_log_id());
    assert_eq!(
        MembershipState::new(
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
            Arc::new(EffectiveMembership::new(Some(log_id(2, s(1), 3)), m23())),
    ...
    assert_eq!(
        vec![
            ...
            Command::UpdateIOProgress {
                ...
                io_id: IOId::new_log_io(Vote::new(2, s(1)).into_committed(), Some(log_id(0, s(1), 0)))
            }
        ],
        eng.output.take_commands()
    );
...
fn test_append_entries_prev_log_id_conflict() -> anyhow::Result<()> {
    ...
    assert_eq!(
        Err(RejectAppendEntries::ByConflictingLogId {
            expect: log_id(2, s(1), 2),
            local: Some(log_id(1, s(1), 2)),
        }),
        res
    );
    assert_eq!(
    ...
    assert_eq!(Vote::new_committed(2, s(1)), *eng.state.vote_ref());
    assert_eq!(Some(&log_id(1, s(1), 1)), eng.state.last_log_id());
    assert_eq!(
        MembershipState::new(
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
...
fn test_append_entries_prev_log_id_is_committed() -> anyhow::Result<()> {
    ...
    let res = eng.append_entries(&Vote::new_committed(2, s(1)), Some(log_id(0, s(1), 0)), vec![
        blank_ent(1, 1),
        blank_ent(2, 2),
    ]);
    ...
    assert_eq!(Vote::new_committed(2, s(1)), *eng.state.vote_ref());
    assert_eq!(Some(&log_id(2, s(1), 2)), eng.state.last_log_id());
    assert_eq!(
        MembershipState::new(
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
...
fn test_append_entries_prev_log_id_not_exists() -> anyhow::Result<()> {
    ...
    let res = eng.append_entries(&Vote::new_committed(2, s(1)), Some(log_id(2, s(1), 4)), vec![
        blank_ent(2, 5),
        blank_ent(2, 6),
    ]);
...
fn test_append_entries_conflict() -> anyhow::Result<()> {
    ...
    let resp = eng.append_entries(&Vote::new_committed(2, s(1)), Some(log_id(1, s(1), 1)), vec![
        blank_ent(1, 2),
        Entry::new_membership(log_id(3, s(1), 3), m34()),
    ]);
    ...
    assert_eq!(Vote::new_committed(2, s(1)), *eng.state.vote_ref());
    assert_eq!(Some(&log_id(3, s(1), 3)), eng.state.last_log_id());
    assert_eq!(
        MembershipState::new(
            Arc::new(EffectiveMembership::new(Some(log_id(1, s(1), 1)), m01())),
            Arc::new(EffectiveMembership::new(Some(log_id(3, s(1), 3)), m34())),
        ),
        eng.state.membership_state
    );
    assert_eq!(ServerState::Learner, eng.state.server_state);
    assert_eq!(
        vec![
            ...
            Command::TruncateLog {
                since: log_id(2, s(1), 3)
            },
            Command::AppendInputEntries {
                committed_vote: Vote::new(2, s(1)).into_committed(),
                entries: vec![Entry::new_membership(log_id(3, s(1), 3), m34())]
            },
        ],
        eng.output.take_commands()
    );
