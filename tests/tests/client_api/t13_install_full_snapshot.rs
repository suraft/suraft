...
async fn install_full_snapshot() -> anyhow::Result<()> {
    ...
    tracing::info!(log_index, "--- fails to install snapshot with smaller vote");
    {
        ...
        let resp = n1.install_full_snapshot(Vote::new(0, s(0)), snap.clone()).await?;
        assert_eq!(
    ...
    );
    {
        ...
        let resp = n1.install_full_snapshot(Vote::new_committed(1, s(0)), snap.clone()).await?;
    ...
    tracing::info!(log_index, "--- succeed to install snapshot on node-2 with higher vote");
    {
        ...
        let resp = n2.install_full_snapshot(Vote::new_committed(1, s(0)), snap.clone()).await?;
        assert_eq!(Vote::new_committed(1, s(0)), resp.vote,);
